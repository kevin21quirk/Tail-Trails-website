...
function buildDoc(drawFn) {
  return new Promise(async (resolve, reject) => {
    const doc = new PDFDocument({ size: 'A4', margin: 0, info: { Creator: 'Tracks & Trails by Alyssia' } });
    const chunks = [];
...
function drawHeader(doc, logoImg, docType, metaLines) {
  ...
  doc.font('Helvetica-Bold').fontSize(19).fillColor(GREEN).text('Tracks & Trails by Alyssia', textX, M + 2);
...
function drawTotals(doc, y, subtotal, total) {
  ...
  doc.font('Helvetica').fontSize(7.5).fillColor(GREY_PALE)
     .text('VAT not applicable \u2014 Tracks & Trails by Alyssia is not VAT registered', 0, y, { width: PW - M, align: 'right' });
  return y + 20;
}
...
async function generateInvoicePdf(inv) {
  ...
  const logoImg = await fetchImageBuffer('https://tailsandtrails.im/Public/tracksandtrailslogo.png');
  ...
  return buildDoc(async (doc) => {
    ...
    drawFooter(doc, y,
      'Thank you for choosing Tracks & Trails by Alyssia!',
