...
function buildDoc(drawFn) {
  return new Promise(async (resolve, reject) => {
    const doc = new PDFDocument({ size: 'A4', margin: 0, info: { Creator: 'Tails & Trails' } });
    const chunks = [];
...
function drawHeader(doc, logoImg, docType, metaLines) {
  ...
  doc.font('Helvetica-Bold').fontSize(19).fillColor(GREEN).text('Tails & Trails', textX, M + 2);
...
function drawTotals(doc, y, subtotal, total) {
  ...
  doc.font('Helvetica').fontSize(7.5).fillColor(GREY_PALE)
     .text('VAT not applicable \u2014 Tails & Trails is not VAT registered', 0, y, { width: PW - M, align: 'right' });
  return y + 20;
}
...
async function generateInvoicePdf(inv) {
  ...
  const logoImg = await fetchImageBuffer('https://tailsandtrails.im/Public/logo.png');
  ...
  return buildDoc(async (doc) => {
    ...
    drawFooter(doc, y,
      'Thank you for choosing Tails & Trails!',
